...
本规格书是「灵镜创影 (FrameMind Studio)」的核心产品设计文档，定义了平台全部功能模块的详细设计、技术架构、数据模型、API 接口和开发路线图。
...
**灵镜创影** 是一个面向个人创作者和小型团队的 **AI 原生影视创作平台**。平台以 **AI Agent** 为创意大脑，结合多模态 AI 技术，提供从"灵感孵化 → 剧本创作 → 分镜设计 → 形象生成 → 视频合成 → 音频制作 → 成片导出"的端到端全链路闭环。
...
1. **🧠 Agent 驱动的创意引擎**：基于 DeepAgents 框架的多智能体编剧室，具备长期记忆、工具链调用和专家技能，实现真正的"人机共创"
...
灵镜创影采用 **DeepAgents** 作为核心 Agent 编排框架。DeepAgents 是 LangChain 生态中面向深度自主 Agent 的框架，基于 LangGraph 构建。
...
**实现方式**：利用 DeepAgents 的 `interrupt()` 原语，在 StateGraph 中插入人类审核节点。Agent 执行到审核节点时自动暂停，等待用户通过 WebSocket 反馈后恢复执行。
...
```json
{
  ...
  "properties": {
    ...
    "totalEpisodes": { "type": "integer", "description": "总集数" },
    "episodes": {
      "type": "array",
      "items": {
        "type": "object",
              ...
              "properties": {
                ...
                "beats": {
                  ...
                  "items": {
                    ...
                    "properties": {
                      "beatId": { "type": "string" },
                      "type": { "type": "string", "enum": ["action", "dialogue", "emotion", "transition"] },
                      "content": { "type": "string" },
                      "character": { "type": "string" },
                      "emotion": { "type": "string" },
                      "cameraSuggestion": { "type": "string" },
...
```json
{
  ...
  "audio": {
    ...
    "sfx": ["office_ambience", "keyboard_typing"]
  },
  "transition": "cut"
}
```
  ...
  "shots": [
    {
      ...
      "character_positions": {
        "char_001": { "x": 0.3, "y": 0.5, "facing": "right" },
        "char_002": { "x": 0.7, "y": 0.5, "facing": "left" }
      },
      "audio": {
        "dialogue_lines": [
          { "character": "陈昊", "text": "这个方案不行，推翻重来。", "emotion": "轻蔑" }
        ],
        "bgm_mood": "tense",
        "sfx": ["door_slam"]
      },
...
// 客户端 → 服务端: 发送消息
{
  ...
  "payload": {
    ...
    "target_agents": ["showrunner", "script_doctor"],
    "context": { "episode": 3, "focus": "turning_point" }
  }
}
...
// 服务端 → 客户端: Agent 思考中
{
  ...
  "payload": {
    ...
    "tool_calls": [
      { "tool": "memory_retrieve", "args": { "query": "第1-2集剧情摘要" } }
    ]
  }
}
...
```json
{
  ...
  "properties": {
    ...
    "mainPlotPoints": { "type": "array", "items": { "type": "string" } },
...
```json
{
  ...
  "properties": {
    "id": { "type": "string" },
    "name": { "type": "string" },
    "roleType": { "type": "string", "enum": ["protagonist", "antagonist", "supporting", "minor"] },
...
```json
{
  ...
  "properties": {
    "shotId": { "type": "string" },
    "shotType": { "type": "string", "enum": ["establishing", "action", "dialogue", "reaction", "montage"] },
    "visualPrompt": { "type": "string" },
    "camera": {
      "type": "object",
      "properties": {
    ...
    "charactersInShot": { "type": "array", "items": { "type": "string" } },
    "audio": {
      "type": "object",
      "properties": {
        "dialogueLines": { "type": "array" },
        "bgmMood": { "type": "string" },
...
当前架构采用**纯 Python 栈**：FastAPI 同时承担 REST API 网关和 Agent 编排职责，DeepAgents（LangChain/LangGraph）嵌入同一进程。评估是否有必要引入 Java Spring Boot 作为独立的业务后端，将 Python 限定为纯 AI 引擎。
