...
**Decision**: 在 PipelineOrchestrator 的 Agent 完成回调中异步触发标题生成。采用智能核心主题提取：去除常见中英文请求前缀（"请帮我"、"Help me" 等），保留核心主题内容（最多 40 字符）。失败时回退到时间戳格式。新增 `title_source` 列区分自动/手动标题，防止自动生成覆盖用户手动编辑的标题。

**Rationale**:
- AgentEndEvent 标志着一次完整对话结束，是生成标题的最佳时机
- 智能提取比原始截取产生更有意义的标题（"主角背景故事" vs "请帮我设计主角的背景故事，包括他的童年经历..."）
- `title_source` 列提供可靠的手动/自动区分（相比仅检查 title 是否为 null）
- 异步执行不阻塞主流程，无额外 LLM 调用成本

**Alternatives considered**:
- 方案 B：调用 LLM 生成摘要标题 → 增加延迟和成本，当前场景不需要
- 方案 C：原始截取前 50 字符 → 产生低质量标题（包含请求前缀），不符合 spec 的"有意义标题"要求
- 方案 D：仅检查 title != null 决定是否覆盖 → 不稳健，无法区分自动标题与手动标题
